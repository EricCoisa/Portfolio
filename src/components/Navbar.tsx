...
const Navbar = () => {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${(isScrolled || isMobileMenuOpen) ? 'bg-background/80 backdrop-blur-lg border-b border-border' : 'bg-transparent'
        }`}
    >
